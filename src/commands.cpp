...
bool handle_invalid_match(Match *current_match) {
 ...
 * every command.
 */
bool handle_empty_match(Match *current_match) {
    if(handle_invalid_match(current_match->error)) {
        return true;
    };
...
#define CHECK_MOVE_CON_TO_WORKSPACE                                                          \
    do {                                                                                     \
        if(handle_empty_match(current_match)) { return; }                                                                  \
        if (TAILQ_EMPTY(&owindows)) {                                                        \
            ...
            while (current) {                                                                \
                owindow *next = TAILQ_NEXT(current, owindows);                               \
                                                                                             \
                if (current->con->type == Con::type::CT_WORKSPACE && !con_has_children(current->con)) { \
...
static void cmd_resize_floating(Match *current_match, CommandResultIR *cmd_output, const char *direction_str, Con *floating_con, int px) {
...
namespace {
bool cmd_resize_tiling_direction(Match *current_match, CommandResultIR *cmd_output, Con *current, const char *direction, int px, int ppt) {
    Con *second = NULL;
    Con *first = current;
...
namespace {
bool cmd_resize_tiling_width_height(Match *current_match, CommandResultIR *cmd_output, Con *current, const char *direction, int px, double ppt) {
...
namespace {
bool resize_set_tiling(Match *current_match, CommandResultIR *cmd_output, Con *target, orientation_t resize_orientation, bool is_ppt, long target_size) {
...
namespace {
int border_width_from_style(border_style_t border_style, long border_width, Con *con) {
 ...
 *
 */
void cmd_append_layout(Match *current_match, CommandResultIR *cmd_output, const char *cpath) {
    ...
    if (content == JSON_CONTENT_WORKSPACE) {
        ...
        while (parent->type != Con::type::CT_WORKSPACE && !con_accepts_window(parent))
 ...
 *
 */
void cmd_workspace_number(Match *current_match, CommandResultIR *cmd_output, const char *which, const char *_no_auto_back_and_forth) {
 ...
 *
 */
void cmd_workspace_name(Match *current_match, CommandResultIR *cmd_output, const char *name, const char *_no_auto_back_and_forth) {
    const bool no_auto_back_and_forth = (_no_auto_back_and_forth != NULL);
...
void cmd_move_con_to_output(Match *current_match, CommandResultIR *cmd_output, const char *name, bool move_workspace) {
    ...
    if (name) {
        ...
        auto *co = create_struct<user_output_name>();
...
void cmd_split(Match *current_match, CommandResultIR *cmd_output, const char *direction) {
    ...
    TAILQ_FOREACH (current, &owindows, owindows) {
        ...
        if (direction[0] == 't') {
            ...
            if (current->con->type == Con::type::CT_WORKSPACE) {
...
void cmd_kill(Match *current_match, CommandResultIR *cmd_output, const char *kill_mode_str) {
    ...
    kill_window kill_mode;
    if (strcmp(kill_mode_str, "window") == 0)
        kill_mode = kill_window::KILL_WINDOW;
    else if (strcmp(kill_mode_str, "client") == 0)
        kill_mode = kill_window::KILL_CLIENT;
    else {
 ...
 *
 */
void cmd_focus_direction(Match *current_match, CommandResultIR *cmd_output, const char *direction_str) {
    if(handle_empty_match(current_match)) { return; }
 ...
 *
 */
void cmd_focus_sibling(Match *current_match, CommandResultIR *cmd_output, const char *direction_str) {
    if(handle_empty_match(current_match)) { return; }
    CMD_FOCUS_WARN_CHILDREN;
    ...
    TAILQ_FOREACH (current, &owindows, owindows) {
        ...
        if (next) {
            if (next->type == Con::type::CT_WORKSPACE) {
...
void cmd_focus_window_mode(Match *current_match, CommandResultIR *cmd_output, const char *window_mode) {
    ...
    TAILQ_FOREACH (current, &(ws->focus_head), focused) {
        if ((to_floating && current->type != Con::type::CT_FLOATING_CON) ||
            (!to_floating && current->type == Con::type::CT_FLOATING_CON))
            continue;
...
void cmd_title_window_icon(Match *current_match, CommandResultIR *cmd_output, const char *enable, int padding) {
    ...
    if(handle_empty_match(current_match)) {
        return;
    };
...
void cmd_rename_workspace(Match *current_match, CommandResultIR *cmd_output, const char *old_name, const char *new_name) {
    ...
    Con *previously_focused_content = focused->type == Con::type::CT_WORKSPACE ? focused->parent : NULL;
...
void cmd_bar_mode(Match *current_match, CommandResultIR *cmd_output, const char *bar_mode, const char *bar_id) {
    auto mode = Barconfig::mode::M_DOCK;
    bool toggle = false;
    if (strcmp(bar_mode, "dock") == 0)
        mode = Barconfig::mode::M_DOCK;
    else if (strcmp(bar_mode, "hide") == 0)
        mode = Barconfig::mode::M_HIDE;
    else if (strcmp(bar_mode, "invisible") == 0)
        mode = Barconfig::mode::M_INVISIBLE;
    ...
    TAILQ_FOREACH (current, &barconfigs, configs) {
        ...
        if (toggle) {
            ...
            // FIXME: modes are 3, are we only iterating between M_DOCK and M_HIDE?
        }
...
void cmd_bar_hidden_state(Match *current_match, CommandResultIR *cmd_output, const char *bar_hidden_state, const char *bar_id) {
    auto hidden_state = Barconfig::hidden_state::S_SHOW;
    bool toggle = false;
    if (strcmp(bar_hidden_state, "hide") == 0)
        hidden_state = Barconfig::hidden_state::S_HIDE;
    else if (strcmp(bar_hidden_state, "show") == 0)
        hidden_state = Barconfig::hidden_state::S_SHOW;
    ...
    TAILQ_FOREACH (current, &barconfigs, configs) {
        ...
        if (toggle) {
            if(current->hidden_state == Barconfig::hidden_state::S_HIDE) {
                hidden_state = Barconfig::hidden_state::S_SHOW;
            } else {
                hidden_state = Barconfig::hidden_state::S_HIDE;
            }
        ...
        if (current->hidden_state != hidden_state) {
